...
pub fn get_aws_config() -> Config {
    ...
    let credentials_profiles: Vec<String> = credentials
        .clone()
        .into_keys()
        .map(|profile| String::from(profile))
        .collect();

    let long_term_credentials_profiles = credentials_profiles
        .iter()
        .filter(|&item| {
            let credential: &Credential = credentials.get(item).unwrap();
...
pub fn get_profile_list(config: &HashMap<String, Profile>, profile_name: &String) -> Vec<String> {
    let mut profiles: Vec<String> = Vec::new();
    profiles.push(profile_name.clone());

    let mut profile_config: Option<Profile> = Some(config.get(profile_name).unwrap().clone());
    while profile_config.is_some() && profile_config.as_ref().unwrap().source_profile.is_some() {
        let source_profile: Option<String> = profile_config.clone().unwrap().source_profile;
        if source_profile.is_some()
            && profiles
                .iter()
                .any(|item| item == source_profile.as_ref().unwrap())
        {
            panic!(
                "Loop in profiles: {} {}",
                profiles.join(", "),
                source_profile.unwrap()
            );
        }
...
fn is_single_role_assuming_profile(
    ...
    match profile.role_arn {
        Some(_) => credentials_profiles
            .iter()
            .any(|p| p == credentials_profile),
        _ => false,
    }
}
...
pub fn get_cachable_profiles(
    ...
    // TODO implement
    let mfa_profiles: Vec<String> = config
        .clone()
        .into_keys()
        .filter(|profile_name| {
            ...
            let long_term_credentials_profile =
                format!("{short_term_credentials_profile}::source-profile");
            long_term_credentials_profiles
                .iter()
                .any(|p| p == &long_term_credentials_profile || p == short_term_credentials_profile)
        })
        .collect();
    ...
    let to_remove: Vec<String> = config
        .clone()
        .into_keys()
        .filter(|profile_name| !mfa_profiles.iter().any(|p| p == profile_name))
        .collect();

    let to_keep: Vec<String> = config
        .clone()
        .into_keys()
        .filter(|profile_name| !to_remove.iter().any(|p| p == profile_name))
        .collect();
